...
import copy
import time
from crazyflie_control.motioncapture_4_2 import DataDescriptions
from crazyflie_control.motioncapture_4_2 import MoCapData
...
class NatNetClient:
    ...
    def __create_data_socket(self, port):
        ...
        if self.use_multicast:
            ...
            result.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, socket.inet_aton(
                self.multicast_address) + socket.inet_aton(self.local_ip_address))
            try:
    ...
    def __unpack_rigid_body(self, data, major, minor, rb_num):
        ...
        offset += 12
        trace_mf("\tPosition    : [%3.2f, %3.2f, %3.2f]" %
                 (pos[0], pos[1], pos[2]))

        rot = Quaternion.unpack(data[offset:offset+16])
        offset += 16
        trace_mf("\tOrientation : [%3.2f, %3.2f, %3.2f, %3.2f]" % (
...
# Unpack Mocap Data Functions
    ...
    def __unpack_legacy_other_markers(self, data, packet_size, major, minor):
        ...
        if (other_marker_count > 0):
            ...
            for j in range(0, other_marker_count):
                pos = Vector3.unpack(data[offset:offset+12])
                offset += 12
                trace_mf("\tMarker %3.1d : [x=%3.2f,y=%3.2f,z=%3.2f]" % (
                    j, pos[0], pos[1], pos[2]))
    ...
    def __unpack_marker_set_data(self, data, packet_size, major, minor):
        ...
        for i in range(0, marker_set_count):
            ...
            # Model name
            model_name, separator, remainder = bytes(
            ...
            for j in range(0, marker_count):
                if (len(data) < (offset+12)):
                    print("WARNING: Early return.  Out of data at marker ",
                          j, " of ", marker_count)
                    offset = len(data)
                    return offset, marker_set_data
                    break
    ...
    def __unpack_labeled_marker_data(self, data, packet_size, major, minor):
        ...
        if ((major == 2 and minor > 3) or major > 2):
            ...
            for lm_num in range(0, labeled_marker_count):
                ...
                size = FloatValue.unpack(data[offset:offset+4])
                offset += 4
                trace_mf("%3.1d ID     : [MarkerID: %3.1d] [ModelID: %3.1d]" % (
                    lm_num, marker_id, model_id))
                trace_mf("    pos  : [%3.2f, %3.2f, %3.2f]" %
                         (pos[0], pos[1], pos[2]))
    ...
    def __unpack_device_data(self, data, packet_size, major, minor):
        ...
        if (major == 2 and minor >= 11) or (major > 2):
            ...
            for i in range(0, device_count):
                ...
                trace_mf("\tDevice %3.1d      ID: %3.1d Num Channels: %3.1d" % (
                ...
                for j in range(0, device_channel_count):
                    ...
                    if n_frames_show < device_channel_frame_count:
                        out_string += " showing %3.1d of %3.1d frames" % (
                            n_frames_show, device_channel_frame_count)
                    trace_mf("%s" % out_string)
    ...
    def __unpack_frame_suffix_data(self, data, packet_size, major, minor):
        ...
        if ((packet_size-offset) <= 0):
            ...
            if major >= 3:
                ...
                trace_mf("Mid-exposure timestamp         : %3.1d" %
                ...
                trace_mf("Camera data received timestamp : %3.1d" %
                         stamp_data_received)

                stamp_transmit = int.from_bytes(
                    data[offset:offset+8], byteorder='little',  signed=True)
                offset += 8
                trace_mf("Transmit timestamp             : %3.1d" %
                         stamp_transmit)
            ...
            if major >= 4 and minor >= 1:
                ...
                # trace_mf("%s"%out_string)
                trace_mf("Precision timestamp (sec)      : %3.1d" %
                         prec_timestamp_secs)
                offset += 4
                ...
                prec_timestamp_frac_secs = int.from_bytes(
                    data[offset:offset+4], byteorder='little',  signed=True)
                trace_mf("Precision timestamp (frac sec) : %3.1d" %
    ...
    def __unpack_marker_set_description(self, data, major, minor):
        ...
        if (marker_count > 0):
            for i in range(0, marker_count):
                name, separator, remainder = bytes(
                    data[offset:]).partition(b'\0')
                offset += len(name) + 1
    ...
    def __unpack_rigid_body_description(self, data, major, minor):
        ...
        trace_dd("\tPosition          : [%3.2f, %3.2f, %3.2f]" % (
            pos[0], pos[1], pos[2]))
        ...
        if (major >= 4 and minor >= 2) or (major == 0):
            ...
            offset += 16
            trace_dd("\tRotation          : [%3.2f, %3.2f, %3.2f, %3.2f]" % (
    ...
    def __unpack_force_plate_description(self, data, major, minor):
        ...
        if major >= 3:
            ...
            trace_dd("\tOrigin : [%3.2f, %3.2f, %3.2f]" %
            ...
            for i in range(0, 12):
                ...
                trace_dd("\t%3.1d %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e" % (i, cal_matrix_row[0], cal_matrix_row[1], cal_matrix_row[2], cal_matrix_row[
                         3], cal_matrix_row[4], cal_matrix_row[5], cal_matrix_row[6], cal_matrix_row[7], cal_matrix_row[8], cal_matrix_row[9], cal_matrix_row[10], cal_matrix_row[11]))
    ...
    def __unpack_data_descriptions(self, data: bytes, packet_size, major, minor):
        ...
        for i in range(0, dataset_count):
            ...
            if data_type == 0:
                ...
                print("\t" + str(i+1) + " datasets processed of " +
                      str(dataset_count))
                print("\t " + str(offset) +
                      " bytes processed of " + str(packet_size))
                print("\tPACKET DECODE STOPPED")
                return offset
            offset += offset_tmp
            data_descs.add_data(data_tmp)
            trace_dd("\t" + str(i+1) + " datasets processed of " +
                     str(dataset_count))
            trace_dd("\t " + str(offset) +
                     " bytes processed of " + str(packet_size))
    ...
    def __unpack_server_info(self, data, packet_size, major, minor):
        ...
        self.__application_name, separator, remainder = bytes(
            data[offset: offset+256]).partition(b'\0')
        ...
        trace_mf("NatNetVersion ", str(self.__nat_net_stream_version_server[0]), " ", str(self.__nat_net_stream_version_server[1]), " ", str(
            self.__nat_net_stream_version_server[2]), " ", str(self.__nat_net_stream_version_server[3]))

        trace_mf("ServerVersion ", str(self.__server_version[0]), " ", str(
            self.__server_version[1]), " ", str(self.__server_version[2]), " ", str(self.__server_version[3]))
        return offset
    ...
    def __command_thread_function(self, in_socket, stop, gprint_level, thread_option):
        ...
        buffer_list_size = 4
        buffer_list = [bytearray(recv_buffer_size)
                       for _ in range(buffer_list_size)]
        ...
        while not stop():
            ...
            try:
                ...
                buffer_list_recv_index = (
                    buffer_list_recv_index + 1) % buffer_list_size
            except socket.error as msg:
                if stop():
    ...
    def __process_message(self, data: bytes, print_level=0):
        ...
        if show_nat_net_version:
            trace("NatNetVersion ", str(self.__nat_net_requested_version[0]), " ", str(self.__nat_net_requested_version[1]), " ", str(
                self.__nat_net_requested_version[2]), " ", str(self.__nat_net_requested_version[3]))
    ...
    def send_command(self, command_str):
        ...
        while nTries:
            ...
            ret_val = self.send_request(
                self.command_socket, self.NAT_REQUEST, command_str,  (self.server_ip_address, self.command_port))
            if (ret_val != -1):
                break
        return ret_val
    ...
    def run(self, thread_option):
        ...
        self.data_thread = Thread(target=self.__data_thread_function, args=(
            self.data_socket, lambda: self.stop_threads, lambda: self.print_level, ))
        self.command_thread = Thread(target=self.__command_thread_function, args=(
            self.command_socket, lambda: self.stop_threads, lambda: self.print_level, thread_option,))
        if thread_option == 'd':
