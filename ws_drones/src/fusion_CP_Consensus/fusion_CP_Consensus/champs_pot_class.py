...
#         nextStep=np.append(nextStep,2*abs(goal.point.z-pose.theta))
...
class CP():
    ...
    def set_next_step(self, goal, pose, obstacles):
        ...
        if np.linalg.norm(F) > 0:
            Kpas = np.clip(self.err_min_obs*self.Kpas_err, self.Kpas_min, self.Kpas_max)
            diff = Kpas - self.Kpas_old
