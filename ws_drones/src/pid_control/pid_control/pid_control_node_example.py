...
from pid_control.pid_class import PID #on importe la classe dans laquelle on a délocalisé le calcul de PID
import math
...
class PIDCOntrolNode(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Pose,'/turtle1/pose', self.listener_cb_pose,10)
        self.subscription1 = self.create_subscription(Pose,'/turtle1/pose_d', self.listener_cb_pose_d,10)
...
def main(args=None):
    ...
    node = PIDCOntrolNode()  # Création du nœud
